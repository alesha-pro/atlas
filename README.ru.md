...
У каждой модели есть прямая ссылка `?model=<slug>`: страницу можно кидать
уже открытой на нужном чекпоинте. Интерфейс стартует в тёмной теме,
переключатель в шапке запоминается в браузере.
